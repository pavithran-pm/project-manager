  ...
  return (
    <>
      <div
        ...
        className={`group/card relative shrink-0 cursor-pointer rounded-lg border bg-white p-2.5 shadow-[0_1px_2px_rgba(42,46,52,0.05)] transition-shadow hover:shadow-md ${
          selected ? 'border-brand ring-1 ring-brand' : 'border-line hover:border-line-strong'
        } ${dragging ? 'opacity-40' : ''} ${justMoved ? 'animate-pop-in' : ''}`}
      >
        {over && (
          <div className="pointer-events-none absolute inset-x-1 -top-1 z-10 h-0.5 rounded-full bg-brand" />
        )}
        {/* Select checkbox (top-left, on hover or when selecting) */}
        <button
          onClick={(e) => {
            e.stopPropagation()
          ...
          className={`absolute top-1.5 left-1.5 z-10 h-4 w-4 items-center justify-center rounded-[4px] border ${
            selected
              ? 'flex border-brand bg-brand'
              : `border-line-strong bg-white ${anySelected ? 'flex' : 'hidden group-hover/card:flex'}`
          }`}
        >
...
function BoardColumn({
  ...
  return (
    <section
      ...
      className={`flex max-h-full w-[280px] shrink-0 flex-col rounded-xl p-1.5 ${tint} ${
        dropActive ? 'bg-[#f0edfc] ring-2 ring-brand/50 ring-inset' : ''
      }`}
    >
